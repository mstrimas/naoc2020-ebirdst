...
To prepare data for comparing Status and Trends data across multiple species. We'll compute proportion of population trajectories for a suite of species.

## Introduction {#traj-intro}

Comparing Status and Trends results between species requires extra caution because the models give _relative_ rather than absolute abundance. Therefore the raw data cannot be directly compared between species because, for example, species differ in their detectability. To address this, we'll convert relative abundance to proportion of population for each species by dividing by the total abundance summed across the entire range. In addition, in this lesson we'll look at the temporal dimension of these data, plotting trajectories through time for a given region.
...
Our goal is to produce trajectories comparing the proportion of each species' population within Kansas. For each species we need to:
...
The below function performs these steps for a given species. Iterating over the full set of species is fairly computationally intensive, taking about 30 minutes on a modern laptop.
